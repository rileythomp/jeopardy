...
void prepare_jeopardy_queries(pqxx::connection& conn) {
  conn.prepare("select_all_questions", 
               "SELECT * FROM QUESTIONS WHERE DAILY_DOUBLE = 'no' AND VALUE != 0");
  conn.prepare("select_by_value",
               "SELECT * FROM QUESTIONS WHERE VALUE = $1 AND DAILY_DOUBLE = 'no'");
  conn.prepare("select_by_category",
               "SELECT * FROM QUESTIONS WHERE CATEGORY LIKE $1 AND DAILY_DOUBLE = 'no' AND VALUE != 0 ORDER BY CATEGORY DESC");
  conn.prepare("select_by_category_and_value",
               "SELECT * FROM QUESTIONS WHERE CATEGORY LIKE $1 AND VALUE = $2 AND DAILY_DOUBLE = 'no' ORDER BY CATEGORY DESC");
}
